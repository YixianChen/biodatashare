...
public class LineChart {
	...
	public LineChart(Context context, List<String> y, long timeFrom, String x,
			String name) {
	...
	public GraphicalView getChartView() {
		...
		for (String x_String : data_strings) {
			...
			if (dataMap.isEmpty()) {
				showDialog("this is no entire data during that time !!! Are you goning to download them? ");
			...
			for(long time : tagTimeList){
				...
				_render.setPointStyle(PointStyle.SQUARE);
				_render.setFillPoints(true);
			...
			mRender.setChartTitle("Line Chart");
			...
			tagMap = gd.getTagMap(timeFrom, time_string);
			...
			if (tagMap.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					if(!entry.getValue().equals("Unknown")){
						...
						mRender.addXTextLabel(Double.valueOf(String.valueOf(entry.getKey())), String.valueOf(entry.getValue()));
						mRender.setLabelsTextSize(15);
						break;
					}
			...
			return ChartFactory.getLineChartView(context, dataset, mRender);
		}
